...
import { musicStatus, musicTitle, musicArtist } from '../../shared/music.js'
...
function LeftSection() {
  ...
  const Music = Widget.Box({
    ...
    children: [
      Widget.Label({
        label: musicTitle.bind().transform(t => musicStatus.value === 'Stopped' ? '󰝛 ' : `󰝚 ${t} - `),
        maxWidthChars: 35,
        truncate: 'end'
      }),
      Widget.Label({
        label: musicArtist.bind().transform(artist => musicStatus.value === 'Stopped' ? '' : artist),
        maxWidthChars: 15,
        truncate: 'end'
      })
    ]
